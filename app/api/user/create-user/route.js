...
export async function POST(request) {
  try {
    ...
    if (!firstName || !lastName || !email || !username) {
      return NextResponse.json(
        {
          error: "All fields are required (firstName, lastName, email, username)",
        },
        { status: 400 }
      );
    }
    ...
    let response;
    try {
      response = await fetch(`${API_BASE_URL}/viewer/create`, {
        method: "POST",
        headers: {
    ...
    if (data && data.status === true) {
      ...
      // Store is_creator: 0 in cookies after successful user creation
      response.cookies.set('is_creator', '0', {
        httpOnly: false,
